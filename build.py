...
def minify_html(html: str) -> str:
    """Минифицирует HTML: убирает множественные пустые строки, лишние пробелы."""
    # Нормализуем line endings (Windows \r\n → \n)
    html = html.replace('\r\n', '\n').replace('\r', '\n')
    # Убираем trailing пробелы/табы на каждой строке
    html = re.sub(r'[ \t]+$', '', html, flags=re.MULTILINE)
    # Схлопываем 3+ переносов подряд → двойной перенос
    html = re.sub(r'\n{3,}', '\n\n', html)
    # Убираем пробелы в начале и конце всего документа
    return html.strip()
