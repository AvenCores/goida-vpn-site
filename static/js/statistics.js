async function loadGitHubStats() {
    ...
    const tText = (key) => escapeHtml(t(key));
    ...
    if (!statsContent.querySelector('.stats-container')) {
        statsContent.innerHTML = `
            <div class="text-center py-6">
                ...
                <p class="mt-2 text-sm text-gray-500">${tText('stats.loading')}</p>
            </div>`;
    }
    ...
    try {
        ...
        const generalTabHtml = `
            <div class="space-y-3 animate-fade-in">
                ...
                <div class="p-3 sm:p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-100 dark:border-blue-800/50 shadow-sm">
                    <div class="text-[10px] text-blue-600 dark:text-blue-400 uppercase font-bold mb-1 tracking-wider">${tText('stats.last_update')}</div>
                    <div class="font-bold text-sm sm:text-base flex items-center gap-2 text-gray-800 dark:text-gray-100 flex-wrap">
                        ...
                        <span>${escapeHtml(dateString)}</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 sm:gap-4">
                    <div class="p-3 sm:p-4 bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-yellow-900/10 dark:to-amber-900/10 rounded-xl border border-yellow-100 dark:border-yellow-800/30 text-center shadow-sm">
                        <div class="text-[10px] text-amber-700 dark:text-amber-500 uppercase font-bold mb-1 tracking-wider">${tText('stats.stars')}</div>
                    ...
                    <div class="p-3 sm:p-4 bg-gradient-to-br from-purple-50 to-fuchsia-50 dark:from-purple-900/10 dark:to-fuchsia-900/10 rounded-xl border border-purple-100 dark:border-purple-800/30 text-center shadow-sm">
                        <div class="text-[10px] text-purple-700 dark:text-purple-400 uppercase font-bold mb-1 tracking-wider leading-tight">${tText('stats.views_14d')}</div>
                ...
                <div class="space-y-2 sm:space-y-3">
                    <div class="flex justify-between items-center p-3 sm:p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm hover:bg-white dark:hover:bg-white/10 transition-colors">
                        <span class="font-bold flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700 dark:text-gray-300 min-w-0">
                            ...
                            <span class="truncate">${tText('stats.clones_14d')}</span>
                        </span>
                    ...
                    <div class="flex justify-between items-center p-3 sm:p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm hover:bg-white dark:hover:bg-white/10 transition-colors">
                        <span class="font-bold flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700 dark:text-gray-300 min-w-0">
                            ...
                            <span class="truncate">${tText('stats.unique_clones_14d')}</span>
                        </span>
                    ...
                    <div class="flex justify-between items-center p-3 sm:p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 shadow-sm hover:bg-white dark:hover:bg-white/10 transition-colors">
                        <span class="font-bold flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-700 dark:text-gray-300 min-w-0">
                            ...
                            <span class="truncate">${tText('stats.unique_visitors_14d')}</span>
                        </span>
                ...
                <div class="mt-4 text-center text-[10px] text-gray-400 uppercase tracking-widest font-bold opacity-60 px-2">
                    ${tText('stats.stats_updated_on_build')}
                </div>
            </div>
        `;
        ...
        if (stats.referrers.length > 0) {
            stats.referrers.forEach(ref => {
                const referrer = escapeHtml(ref.referrer || '');
                referrersTabHtml += `
                    <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 hover:bg-white dark:hover:bg-white/10 transition-colors gap-2">
                        <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                            ...
                            <div class="flex flex-col min-w-0">
                                <span class="font-bold text-xs sm:text-sm text-gray-800 dark:text-gray-200 truncate" title="${referrer}">${referrer}</span>
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wide">${tText('stats.referrer_label')}</span>
                            </div>
                        </div>
                        <div class="text-right shrink-0">
                            <div class="font-black text-xs sm:text-sm text-gray-800 dark:text-gray-100 whitespace-nowrap">${formatNumber(ref.count)} <span class="text-[10px] font-normal text-gray-400">${tText('stats.views_short')}</span></div>
                            <div class="text-[10px] text-gray-400">${formatNumber(ref.uniques)} ${tText('stats.unique_short')}</div>
                        </div>
                    </div>
                `;
            });
            ...
            referrersTabHtml += `
                <div class="text-center py-10 opacity-60">
                    ...
                    <p class="text-sm text-gray-500">${tText('stats.no_referrers')}</p>
                </div>
            `;
        }
        ...
        if (stats.popular_content.length > 0) {
                ...
                contentTabHtml += `
                    <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/10 hover:bg-white dark:hover:bg-white/10 transition-colors gap-2">
                        <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                            ...
                            <div class="flex flex-col min-w-0">
                                <span class="font-bold text-xs sm:text-sm text-gray-800 dark:text-gray-200 truncate" title="${path}">${title}</span>
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wide truncate" title="${path}">${path}</span>
                            </div>
                        </div>
                        <div class="text-right shrink-0">
                            <div class="font-black text-xs sm:text-sm text-gray-800 dark:text-gray-100 whitespace-nowrap">${formatNumber(item.count)} <span class="text-[10px] font-normal text-gray-400">${tText('stats.views_short')}</span></div>
                            <div class="text-[10px] text-gray-400">${formatNumber(item.uniques)} ${tText('stats.unique_short')}</div>
                        </div>
                    </div>
                `;
            });
            ...
            contentTabHtml += `
                <div class="text-center py-10 opacity-60">
                    ...
                    <p class="text-sm text-gray-500">${tText('stats.no_content')}</p>
                </div>
            `;
        }
        contentTabHtml += '</div>';
        ...
        statsContent.innerHTML = `
            <div class="stats-container flex flex-col h-full">
                ...
                <div class="flex-shrink-0 flex space-x-1 bg-gray-100 dark:bg-white/5 p-1 rounded-xl mb-3">
                    <button onclick="switchStatsTab('general', this)" class="stats-tab-btn active flex-1 py-2.5 text-xs font-bold rounded-lg transition-all bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white touch-manipulation">${tText('stats.general')}</button>
                    <button onclick="switchStatsTab('referrers', this)" class="stats-tab-btn flex-1 py-2.5 text-xs font-bold rounded-lg transition-all text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 touch-manipulation">${tText('stats.referrers')}</button>
                    <button onclick="switchStatsTab('content', this)" class="stats-tab-btn flex-1 py-2.5 text-xs font-bold rounded-lg transition-all text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 touch-manipulation">${tText('stats.popular_content')}</button>
                </div>
        ...
        statsContent.innerHTML = `
            <div class="text-center text-red-500 py-6">
                ...
                <p class="font-semibold text-sm">${tText('stats.error_loading')}</p>
                <p class="text-xs mt-1 opacity-75">${escapeHtml(error.message)}</p>
            </div>
        `;
    }
}
